...
2.  Use emojis to make the profile more engaging and visually appealing.
3.  Structure the information into logical sections (e.g., "👋 About Me", "🚀 What I'm Up To", "💻 Tech Stack", "📫 Let's Connect", "📊 GitHub Stats").
4.  For the "Tech Stack" section, create shields.io badges for each technology listed. The technologies are comma-separated. For each technology (e.g., 'React'), create a badge like \`https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB\`. You must find appropriate slugs and hex colors for each technology. If a logo is not available, omit the logo parameter.
5.  Include a section for GitHub stats cards using the user's username. Include both the main stats card and the top languages card. Use the Dracula theme.
...
6. Combine all these elements into a single, cohesive Markdown output.
7.  The entire output MUST be valid Markdown. Do not include any explanations, introductory text, or concluding remarks. Just return the raw Markdown content.
8. If a section has no data (e.g., hobbies is 'Not provided'), be creative. You can either omit the section or add a playful placeholder.
`;
};
